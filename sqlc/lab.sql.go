...
const getLabTestByID = `-- name: GetLabTestByID :one
...
func (q *Queries) GetLabTestByID(ctx context.Context, id nulls.Int64) (*LabTest, error) {
	row := q.db.QueryRowContext(ctx, getLabTestByID, id)
	var i LabTest
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
